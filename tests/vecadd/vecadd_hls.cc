#include "vecadd_hls.h"

#include <stdint.h>

#include <hls_stream.h>

namespace vhlstest {
namespace {

template <typename T>
void vecadd(hls::stream<T>& src0, hls::stream<T>& src1, const int32_t width,
            const int32_t height, hls::stream<T>& dst) {
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
#pragma HLS pipeline II = 1
      const T out = src0.read() + src1.read();
      dst.write(out);
    }
  }
}

}  // namespace
}  // namespace vhlstest

void vecadd_hls(hls::stream<DType>& src0, hls::stream<DType>& src1,
                int32_t width, int32_t height, hls::stream<DType>& dst) {
#pragma HLS dataflow
#pragma HLS interface axis port = src0
#pragma HLS interface axis port = src1
#pragma HLS interface s_axilite port = width bundle = BUS_AXI4LS
#pragma HLS interface s_axilite port = height bundle = BUS_AXI4LS
#pragma HLS interface axis port = dst

  vhlstest::vecadd<DType>(src0, src1, width, height, dst);
}
